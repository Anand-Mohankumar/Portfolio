...
- **`index.html`** — all window markup. Every "app" (Home, Projects, Skills, Terminal, Markdownify) and every nested folder/resource view is a `.window-card` element present in the DOM, shown/hidden by JS. Markdownify's external libs — `turndown` + `turndown-plugin-gfm` (HTML→Markdown) and `marked` (Markdown→HTML) — are **not** in the HTML: `script.js` loads them on demand (pinned versions, `MD_LIBS` / `loadMarkdownLibs`) the first time the Markdownify window opens, so they never block page load.
...
- **Load performance rules** (keep these when editing): images inside windows that are closed at startup must be `loading="lazy"` (windows are hidden with `display:none`, so lazy images are skipped on first load and warmed into the cache after boot by `warmLazyImages`); iframes are `loading="lazy"`; the boot sequence starts on `DOMContentLoaded`, never `load`; don't add `backdrop-filter` to elements with an opaque background or nested inside another blurred element (it's invisible but still costs a re-blur every frame over the animated background).
...
- `infographics/` — image gallery. Full-size PNGs are organized into category subfolders that mirror the website's window nesting: `infographics/AI/{AI Fundamentals, AI Advanced, Claude}/`. `infographics/thumbnails/` mirrors the same subfolder tree (`thumbnails/AI/<Category>/`). In `index.html`, each infographic's thumbnail `<img src>` points at the `thumbnails/AI/<Category>/<file>` path and its `onclick` opens the full-size GitHub raw URL `…/blob/main/infographics/AI/<Category>/<file>?raw=true`. **Whenever folders or files are added to `infographics/`, `index.html` must also be edited so they show up in the UI** (the directory and the site are kept one-to-one):
  - **New image in an existing category** → place the full-size image and its thumbnail under the matching `AI/<Category>/` path in both trees, and add an `infographic-item` (thumbnail `<img src>` with `loading="lazy" decoding="async"` + full-size `onclick`) to the correct category window (`#aiFundamentalsCard` / `#aiAdvancedCard` / `#aiClaudeCard`).
...
**Branded visuals (posts, infographics, slides):** use the project skill `cyberamo-brand-visuals` (`.claude/skills/cyberamo-brand-visuals/`). It packages both guides into an HTML theme, format templates, a headless-browser renderer (`scripts/render.py`, needs only Edge or Chrome) and readability rules, and covers converting the existing `infographics/` images to the brand. Invoke it for any request to create or re-skin a social post, carousel, infographic, banner, slide or deck.
