...
# build-lecture-notes-v2.ps1
# Reads pre-extracted body content from .tmp files and builds final HTML.
...
# ── Read pre-extracted bodies from temp files ─────────────────────────────────
$linuxBody = [System.IO.File]::ReadAllText((Join-Path $BasePath "scripts\linux-body.tmp"), [System.Text.Encoding]::UTF8)
$netBody   = [System.IO.File]::ReadAllText((Join-Path $BasePath "scripts\net-body.tmp"),   [System.Text.Encoding]::UTF8)

Write-Host "Linux body: $($linuxBody.Length) chars"
Write-Host "Net body:   $($netBody.Length) chars"
...
Build-Page `
    ...
    -OutPath     (Join-Path $BasePath "labs-research-and-ctf\lecture-notes\linux-fundamentals\index.html") `
...
Build-Page `
    ...
    -BodyContent $netBody `
    -OutPath     (Join-Path $BasePath "labs-research-and-ctf\lecture-notes\networking-fundamentals\index.html") `
    -CssRelPath  "../../../styles.css"

Write-Host ""
Write-Host "All done. Both lecture note pages rebuilt with scroll fix."
